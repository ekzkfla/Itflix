...
public interface Category_Mapper {
	...
	public int selectByNo(int cg_no);
	
	//카테고리 이름을 이용하여 1개 출력 
	@Select("select * from Category where cg_name = #{cg_name}")
	public int selectByName(String cg_name);
	...
	@Select("select c.cg_name, m.m_image, m.m_name, m.m_actor, m.m_info, m.m_date from movie m join category c on m.cg_no=c.cg_no where m.cg_no=#{cg_no}")
	public int selectByNoMovieList(int cg_no);
